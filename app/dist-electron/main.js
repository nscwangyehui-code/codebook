...
function sanitizeForYaml(obj) {
  return pruneUndefined(obj);
}
function ensureSafeFileName(raw) {
  const trimmed = raw.trim();
  const noControl = Array.from(trimmed).map((ch) => ch.charCodeAt(0) < 32 ? "-" : ch).join("");
  const replaced = noControl.replace(/[<>:"/\\|?*]/g, "-");
  const collapsed = replaced.replace(/-+/g, "-");
  const safe = collapsed.slice(0, 48).replace(/^[.\s-]+|[.\s-]+$/g, "");
  return safe || "未命名";
}
function ensureSafeNoteFileName(fileName) {
...
function noteFolderPath(notesDir, sourceRelativePath) {
  return path.join(notesDir, ...sourceRelativePath.split("/"));
}
async function ensureDir(dirPath) {
  await fs$1.promises.mkdir(dirPath, { recursive: true });
}
async function writeTextFileAtomic(filePath, content) {
  ...
  await fs$1.promises.writeFile(tempPath, content, "utf8");
  try {
    await fs$1.promises.rename(tempPath, filePath);
  } catch (e) {
    const err = e;
    if (err.code !== "EPERM" && err.code !== "EEXIST" && err.code !== "ENOTEMPTY") throw err;
    await fs$1.promises.copyFile(tempPath, filePath);
    await fs$1.promises.unlink(tempPath).catch(() => {
    });
  }
}
async function writeBufferFileAtomic(filePath, buffer) {
  const dir = path.dirname(filePath);
  await ensureDir(dir);
  const tempName = `${path.basename(filePath)}.${crypto.randomBytes(6).toString("hex")}.tmp`;
  const tempPath = path.join(dir, tempName);
  await fs$1.promises.writeFile(tempPath, buffer);
  try {
    await fs$1.promises.rename(tempPath, filePath);
  } catch (e) {
    const err = e;
...
async function ensureMainNote(notesDir, sourceRelativePath, source) {
  const folder = noteFolderPath(notesDir, sourceRelativePath);
  await ensureDir(folder);
  const mainPath = path.join(folder, "main.md");
  try {
    await fs$1.promises.access(mainPath);
    return;
  } catch {
    ...
    const initialBody = "";
    const full = matter$1.stringify(initialBody, sanitizeForYaml(fm));
    await writeTextFileAtomic(mainPath, full);
  }
}
async function listNotes(notesDir, sourceRelativePath) {
  const folder = noteFolderPath(notesDir, sourceRelativePath);
  await ensureDir(folder);
  let fileNames = [];
  try {
    fileNames = await fs$1.promises.readdir(folder);
  } catch {
    fileNames = [];
  }
  ...
  for (const fileName of mdFiles) {
    ...
    try {
      const raw = await fs$1.promises.readFile(filePath, "utf8");
...
async function readNote(notesDir, sourceRelativePath, fileName) {
  const folder = noteFolderPath(notesDir, sourceRelativePath);
  const filePath = path.join(folder, fileName);
  const raw = await fs$1.promises.readFile(filePath, "utf8");
  const parsed = matter$1(raw);
...
async function saveNote(notesDir, sourceRelativePath, fileName, content, patch) {
  const folder = noteFolderPath(notesDir, sourceRelativePath);
  const filePath = path.join(folder, fileName);
  let frontmatter;
  try {
    ...
    const parsed = matter$1(raw);
    const data = parsed.data;
    const source = data.source ?? patch.source;
  ...
  const full = matter$1.stringify(content, sanitizeForYaml(frontmatter));
  await writeTextFileAtomic(filePath, full);
}
async function createNote(notesDir, sourceRelativePath, title, source) {
  const folder = noteFolderPath(notesDir, sourceRelativePath);
  await ensureDir(folder);
  const createdAt = nowIso();
  ...
  const full = matter$1.stringify("", sanitizeForYaml(fm));
  await writeTextFileAtomic(path.join(folder, fileName), full);
  return fileName;
}
async function deleteNote(notesDir, sourceRelativePath, fileName) {
  const folder = noteFolderPath(notesDir, sourceRelativePath);
  ...
  while (current !== root && current.startsWith(root + path.sep)) {
    try {
      const items = await fs$1.promises.readdir(current);
      if (items.length) break;
      await fs$1.promises.rmdir(current);
    } catch {
      break;
    }
...
async function saveImageFromDataUrl(notesDir, dataUrl) {
  ...
  const alreadyExists = await fs$1.promises.access(absPath).then(() => true).catch(() => false);
...
async function scanAllNoteDocs(notesDir) {
  const root = path.resolve(notesDir);
  const ignoreDirNames = /* @__PURE__ */ new Set(["assets", ".git", ".vscode", "node_modules"]);
  const docs = [];
  const stack = [root];
  while (stack.length) {
    ...
    let entries;
    try {
      entries = await fs$1.promises.readdir(current, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const entry of entries) {
      ...
      try {
        const raw = await fs$1.promises.readFile(abs, "utf8");
        const parsed = matter$1(raw);
        const data = parsed.data;
...
async function findGitDir(startDir) {
  ...
  while (current) {
    ...
    if (await exists(dotGit)) {
      const stat = await fs$1.promises.stat(dotGit);
      if (stat.isDirectory()) return dotGit;
      if (stat.isFile()) {
        const content = await fs$1.promises.readFile(dotGit, "utf8");
...
async function readPackedRefsCommit(gitDir, refPath) {
  const packedRefsPath = path.join(gitDir, "packed-refs");
  if (!await exists(packedRefsPath)) return null;
  const packed = await fs$1.promises.readFile(packedRefsPath, "utf8");
...
async function getGitHeadInfo(gitDir) {
  const headPath = path.join(gitDir, "HEAD");
  const head = (await fs$1.promises.readFile(headPath, "utf8")).trim();
  if (head.startsWith("ref:")) {
    ...
    if (await exists(refFile)) {
      headCommit = (await fs$1.promises.readFile(refFile, "utf8")).trim();
    } else {
      headCommit = await readPackedRefsCommit(gitDir, refPath) ?? void 0;
    }
    return { branch, headCommit };
  }
  return { branch: void 0, headCommit: head };
}
async function computeFingerprintFast(dir) {
  const ignoreDirNames = /* @__PURE__ */ new Set(["node_modules", ".git", ".vscode", "dist", "build", "__pycache__"]);
  let fileCount = 0;
  let maxMtimeMs = 0;
  const stack = [dir];
  while (stack.length) {
  ...
  const hash = crypto.createHash("sha256").update(`${path.resolve(dir)}|${fileCount}|${maxMtimeMs}`).digest("hex").slice(0, 12);
  return hash;
}
async function getCodeVersion(sourceDir, filePath) {
  ...
  if (!gitDir) {
    ...
    return {
      ...
      relativePath: filePath ? toPosixRelativePath(sourceDir, filePath) : void 0,
      fingerprint,
      label: void 0
    };
  }
  ...
  if (filePath) {
    try {
      const fileStat = await fs$1.promises.stat(filePath);
      const indexPath = path.join(gitDir, "index");
      if (await exists(indexPath)) {
        const indexStat = await fs$1.promises.stat(indexPath);
...
function resolveSafeChildPath(baseDir, posixRelativePath) {
...
function ensureNotesOutsideSource(sourceDir, notesDir) {
...
function toNoteSource(codeVersion, sourceRelativePath) {
...
ipcMain.handle("path:join", async (_event, payload) => {
  return path.join(payload.base, payload.child);
});
ipcMain.handle("fs:scanDirectory", async (_event, payload) => {
  const sourceDir = path.resolve(payload.sourceDir);
  const ignoreDirNames = /* @__PURE__ */ new Set(["node_modules", ".git", ".vscode", "dist", "build", "__pycache__"]);
  const allowExt = /* @__PURE__ */ new Set([
    ".ts",
    ".tsx",
    ".js",
    ".jsx",
  ...
  const allowNoExt = /* @__PURE__ */ new Set(["makefile", "dockerfile", "license", "readme"]);
  const results = [];
...
ipcMain.handle("fs:readTextFile", async (_event, payload) => {
  ...
  return fs$1.promises.readFile(filePath, "utf8");
});
ipcMain.handle("source:getCodeVersion", async (_event, payload) => {
...
ipcMain.handle("notes:saveImage", async (_event, payload) => {
  return saveImageFromDataUrl(payload.notesDir, payload.dataUrl);
});
ipcMain.handle("notes:readAssetDataUrl", async (_event, payload) => {
  ...
  const buf = await fs$1.promises.readFile(target);
...
ipcMain.handle("search:loadNoteDocs", async (_event, payload) => {
...
ipcMain.handle(
  ...
  async (_event, payload) => {
    ...
    const ignoreDirNames = /* @__PURE__ */ new Set([".git", ".vscode", "node_modules", "dist", "build", "out", ".next", ".turbo"]);
    const allowExt = /* @__PURE__ */ new Set([
      ".ts",
      ".tsx",
      ".js",
    ...
    while (stack.length && results.length < payload.limit) {
      ...
      for (const entry of entries) {
        ...
        let st;
        try {
          st = await fs$1.promises.stat(abs);
        } catch {
          continue;
        }
        ...
        let buf;
        try {
          buf = await fs$1.promises.readFile(abs);
        } catch {
          continue;
        }
...
function normalizeOpenAIChatCompletionsUrl(endpoint) {
...
ipcMain.handle(
  ...
  async (_event, payload) => {
    ...
    if (payload.mode === "ollama") {
      const url2 = payload.endpoint.replace(/\/+$/g, "") + "/api/generate";
      const prompt = `你是“代码伴生笔记”的代码阅读助手。\\n请用中文解释下面代码片段的具体作用、关键变量含义、可能的边界情况。\\n文件：${payload.sourceRelativePath}\\n代码：\\n\\n${selected}\\n`;
      const json22 = await fetchJsonWithTimeout(
        url2,
        {
          method: "POST",
      ...
      const response = typeof (json22 == null ? void 0 : json22.response) === "string" ? json22.response : JSON.stringify(json22);
      return { text: response };
    }
    const url = normalizeOpenAIChatCompletionsUrl(payload.endpoint);
    const apiKey = (_a = payload.apiKey) == null ? void 0 : _a.trim();
    if (!apiKey) throw new Error("未提供 API Key");
    const json2 = await fetchJsonWithTimeout(
      url,
      {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: "user",
              content: `请用中文解释下面代码片段的具体作用、关键变量含义、可能的边界情况。\\n文件：${payload.sourceRelativePath}\\n代码：\\n\\n${selected}\\n`
            }
          ],
          temperature: 0.2
        })
    ...
    const text = (_d = (_c = (_b = json2 == null ? void 0 : json2.choices) == null ? void 0 : _b[0]) == null ? void 0 : _c.message) == null ? void 0 : _d.content;
